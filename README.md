...
- **决策同步**：每次分析都会保存一条历史决策，记录当时价格、AI输出和决策方向；配置 KV 后可跨浏览器同步。
...
TWELVE_DATA_TIME_OFFSET_MINUTES=-120
USD_CNY=7.20
```
...
`TWELVE_DATA_TIME_OFFSET_MINUTES` 用于校准 Twelve Data K线展示时间。当前 `XAU/USD` 返回的 K线时间对国内显示偏快约2小时，因此默认 `-120`。实时报价时间使用 Twelve Data 的 `last_quote_at`，不使用只有日期的 `datetime` 字段。
...
未配置 KV 时，系统仍能分析和画当前图表，但不会跨设备、跨浏览器长期保存历史快照和决策历史。`MARKET_HISTORY_LIMIT` 控制最多保留多少条，默认 1000 条。
