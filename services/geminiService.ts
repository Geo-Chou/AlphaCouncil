...
export async function generateAgentResponse(
  ...
  const prompt = `
    目标标的: ${stockSymbol || 'XAUUSD'} (黄金 / XAUUSD / US$/oz)
    ...
    【严格执行指令】:
    1. 必须优先参考提供的【黄金行情数据】，特别是现价、日内高低点、涨跌幅、点差和人民币换算参考。
    2. 必须围绕黄金交易变量分析：美元、美债实际利率、通胀预期、央行购金、避险需求、ETF/期货资金、技术位。
    3. 国内交易者执行不等同于直接交易 XAUUSD，必须考虑银行金、上海金、黄金ETF、纸黄金或合规平台开户品种的点差、汇率和交易时段。
    4. **输出必须极度精炼、专业，使用Markdown列表格式。**
    5. **严禁废话、客套话，直接给出结论和数据支撑。**
  `;
